...
must_haves:
  ...
  key_links:
    - from: "apps/dashboard/src/pages/Repos.tsx"
      to: "apps/dashboard/src/queries/use-repos.ts"
      via: "useRepos() hook"
      pattern: "useRepos"
...
PageHeader with toggle register form:
...
- Each repo as a card (rounded-lg border bg-card):
...
**3. Update `router.tsx`** — wire `/repos` route to Repos component (replace placeholder).
  ...
  <verify>
`pnpm --filter @lens/dashboard build` succeeds. Repos.tsx exports Repos component. Router maps /repos.
  </verify>
  <done>
Repos page shows repo cards with register form. Re-index works via mutation. Clicking repo navigates to detail. v1 card design preserved (simplified for v2).
  </done>
</task>
...
1. **PageHeader**: Back button (ArrowLeft) -> "/repos", repo name, action buttons (Re-index, Remove with confirm)
...
3. **Overview tab:**
   - Grid cards (`gap-2 @xl/main:grid-cols-2 @3xl/main:grid-cols-3`):
...
5. **File detail Sheet (EXACT v1 design):**
   ...
   - Sections separated by `<Separator />`:
     - Exports: flex-wrap Badge list (variant="secondary", font-mono text-[10px])
...
2. RepoDetail.tsx has two-panel layout (grep for `w-48.*border-r`)
3. File detail Sheet has structural context sections (grep for `Imported by|Imports|Co-changes|Git activity`)
4. Router maps /repos/:repoId to RepoDetail
  </verify>
  <done>
RepoDetail page has two-panel layout with overview and files tabs. Files table shows dense raw table with file metadata. File detail Sheet shows full structural context (exports, imports, importers, git stats, co-changes) with inter-file navigation. v1 design preserved.
  </done>
</task>
...
- Repos page has register form and repo cards
...
<success_criteria>
Repo explorer complete: repos list with register/re-index, repo detail with two-panel file browser, file detail Sheet with imports/importers/git/co-changes. Dense table styling matches v1 exactly. Inter-file navigation works in Sheet.
</success_criteria>
