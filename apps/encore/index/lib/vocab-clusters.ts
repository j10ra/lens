...
const STOPWORDS = new Set([
  ...
  "what", "which", "who", "when", "where", "why", "get", "set", "new",
  "null", "true", "false", "void", "type", "var", "let", "const",
  "return", "import", "export", "default", "class", "function",
  "string", "number", "boolean", "object", "array", "index", "data",
  "value", "result", "item", "list", "name", "id", "key", "src",
  "lib", "app", "test", "spec", "mock", "model", "module", "component",
]);
...
/** Extract unique vocabulary from file metadata */
function extractVocab(
  files: Array<{ path: string; exports: string[] }>,
): { terms: string[]; termToFiles: Map<string, Set<string>> } {
  ...
  for (const f of files) {
    ...
    for (const seg of pathParts) {
      const base = seg.replace(/\.[^.]+$/, ""); // strip extension
  ...
  // Filter terms appearing in too many files (noise) or only 1 file (not useful for clustering)
  ...
  // Cap at MAX_TERMS — keep most frequent
...
/** Single-linkage clustering — pre-compute pairwise similarities, merge greedily */
function agglomerativeCluster(
  ...
  // Collect all above-threshold pairs, sorted by similarity descending
  ...
  // Union-Find for fast cluster membership
...
/** Build vocab clusters for a repo and persist to DB */
export async function buildVocabClusters(repoId: string): Promise<void> {
  // Load file metadata
  const rows: Array<{ path: string; exports: string[] }> = [];
  const cursor = db.query<{ path: string; exports: string | string[] }>`
    SELECT path, exports FROM file_metadata WHERE repo_id = ${repoId}
  `;
  ...
  if (terms.length < 4) return; // not enough vocab to cluster

  // Embed all terms via Voyage
  let embeddings: number[][];
  try {
    embeddings = await embedBatched(terms);
  } catch {
    // Voyage unavailable — skip silently
    return;
  }
  ...
  // Build output: resolve term indices → { terms, files }
  ...
  // Persist
  await db.exec`UPDATE repos SET vocab_clusters = ${JSON.stringify(clusters)}::jsonb WHERE id = ${repoId}`;
}

/** Embed terms in batches — uses larger batch size since terms are short */
async function embedBatched(terms: string[]): Promise<number[][]> {
  const results: number[][] = [];
  for (let i = 0; i < terms.length; i += TERM_BATCH_SIZE) {
    const batch = terms.slice(i, i + TERM_BATCH_SIZE);
    const vecs = await embed(batch, false);
    results.push(...vecs);
  }
  return results;
}
