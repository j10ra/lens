// Static import extraction — regex-based, zero LLM, zero DB migration.
// Dispatches by language to extract local/relative import specifiers,
// resolves to known repo paths, builds bidirectional import graph.
...
/** Extract raw import specifiers from source content, dispatched by language */
...
function extractGo(content: string): string[] {
  ...
  for (const m of content.matchAll(GO_IMPORT_RE)) {
    // Skip stdlib — stdlib paths have no '.'
...
/** Resolve a single import specifier to a known repo path (or null) */
...
function resolveTS(dir: string, spec: string, known: Set<string>): string | null {
  ...
  // Exact match (rare — e.g. .json imports)
...
function resolveGo(spec: string, known: Set<string>): string | null {
  // Match against known paths containing the module segment
...
function resolveRust(dir: string, spec: string, known: Set<string>): string | null {
  // crate:: → from repo root, super:: → parent, self:: → current
  let base: string;
...
/** Build bidirectional import graph from file contents */
export function buildImportGraph(
  ...
  for (const file of files) {
    ...
    const specs = extractImportSpecifiers(file.content, file.language);
    for (const spec of specs) {
      const resolved = resolveImport(file.path, spec, file.language, knownPaths);
      if (resolved && resolved !== file.path) {
        ensure(resolved);
        graph.get(file.path)!.add(resolved);
        graph.get(resolved)!.add(file.path); // bidirectional
      }
    }
  }
...
/** Expand hit paths by 1 level of imports, returns only NEW paths */
export function expandByImports(
  hitPaths: string[],
  ...
  for (const p of hitPaths) {
    const edges = graph.get(p);
    if (!edges) continue;
    for (const neighbor of edges) {
      if (!hitSet.has(neighbor) && !neighbors.includes(neighbor)) {
        neighbors.push(neighbor);
        if (neighbors.length >= maxExpansion) return neighbors;
      }
    }
  }
